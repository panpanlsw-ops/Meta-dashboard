...
overview_data = [
    ...
    ["Reach",          5400000,  6700000, -19.2],
    ["Frequency",         14.6,    13.8,   6.1],
    ["Clicks",          263900,  305700, -13.7],
    ["Leads",             3100,   4600,  -32.4],
    ["Purchases",         2700,   3546,  -23.8],
    ["ROAS",               5.0,    3.94,  26.9],
    ["CPM ($)",           5.48,    5.38,   1.9],
    ["CPC ($)",           0.43,    0.42,   2.4],
    ["CTR (%)",           1.29,    1.28,   0.8],
    ["Conversion Rate (%)", 1.02,  1.50, -32.0],
]
...
camp_headers = ["Campaign Objective","Spend ($)","Impressions","CPM ($)",
                "Clicks","CPC ($)","Leads","Purchases","ROAS","CTR (%)"]
...
daily_headers = ["Date","Spend ($)","Impressions","Reach","Clicks",
                 "Leads","Purchases","ROAS","Frequency"]
...
for i in range(31):
    ...
    leads      = random.randint(60, 150)
    purchases  = random.randint(50, 120)
    roas       = round(random.uniform(3.5, 8.0), 2)
    freq       = round(random.uniform(12, 18), 1)
    ws3.append([d.strftime("%Y-%m-%d"), spend, impr, reach, clicks, leads, purchases, roas, freq])

for i, w in enumerate([14,12,14,12,10,8,12,8,12], 1):
...
adset_headers = ["Ad Set Name","Campaign","Spend ($)","Impressions",
                 "Clicks","CTR (%)","CPC ($)","Leads","Purchases","ROAS"]
...
col_ws4 = [24,22,12,14,10,10,10,10,12,10]
for i, w in enumerate(col_ws4, 1):
    ws4.column_dimensions[get_column_letter(i)].width = w

wb.save("/home/claude/meta_dashboard/meta_ads_data.xlsx")
