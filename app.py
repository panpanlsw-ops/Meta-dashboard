...
META_SVG = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 24" height="16" style="vertical-align:middle">
...
/* Top bar */
.topbar{background:#1877F2;height:50px;display:flex;align-items:center;
  padding:0 20px;gap:12px}
.tb-brand{display:flex;align-items:center;gap:8px}
.tb-brand-name{color:white;font-weight:700;font-size:1rem}
.tb-sep{width:1px;height:24px;background:rgba(255,255,255,0.3)}
.tb-pagetitle{color:white;font-size:0.9rem;font-weight:600}
.tb-right{margin-left:auto;display:flex;align-items:center;gap:10px}
.tb-date{color:rgba(255,255,255,0.85);font-size:0.82rem}
...
.kcell{background:white;border:1px solid #e5e7eb;border-radius:10px;
  padding:16px 14px 12px;position:relative;box-shadow:0 1px 4px rgba(0,0,0,0.06)}
...
.kl{font-size:0.65rem;color:#9ca3af;font-weight:600;text-transform:uppercase;
  letter-spacing:.07em;margin-bottom:6px}
.kv{font-size:1.35rem;font-weight:700;color:#111827;line-height:1.1;margin-bottom:5px}
.kp{font-size:0.72rem;color:#10b981;font-weight:600}
.kn{font-size:0.72rem;color:#ef4444;font-weight:600}

/* Section header + body */
.sh{background:#1877F2;color:white;padding:8px 14px;border-radius:8px 8px 0 0;
  font-weight:600;font-size:0.82rem;display:flex;align-items:center;gap:6px}
.sb{background:white;border:1px solid #e5e7eb;border-top:none;
  border-radius:0 0 8px 8px;padding:14px;margin-bottom:24px}

/* Territory KPI strip */
.tstrip{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;margin-bottom:24px}
.tcell{background:white;border:1px solid #e5e7eb;border-radius:10px;
  padding:14px 14px 12px;text-align:center;box-shadow:0 1px 4px rgba(0,0,0,0.05)}
.ttop{display:block;height:4px;border-radius:3px;margin-bottom:8px}
.tl{font-size:0.65rem;color:#9ca3af;font-weight:600;text-transform:uppercase;
...
st.markdown(f"""
<div class="topbar">
  <div class="tb-brand">{META_SVG}<span class="tb-brand-name">Meta Ads</span></div>
  <div class="tb-sep"></div>
  <span class="tb-pagetitle">{page_titles[st.session_state.page]}</span>
  <div class="tb-right">
...
with nav_col:
    ...
    for key, icon, label in nav_items:
        ...
        if active:
            st.markdown(
                ...
                unsafe_allow_html=True)
            # invisible button to capture click (already active)
        else:
    ...
    st.markdown("<hr style='border-color:rgba(255,255,255,0.08);margin:12px 0'>",
                unsafe_allow_html=True)
    ...
    sel_off = st.selectbox("Office / Territory", off_list, label_visibility="collapsed")
...
with content_col:
    ...
    if st.session_state.page == "overview":
        ...
        st.markdown(
            ...
            "margin:16px 0 18px;display:flex;align-items:center;gap:8px'>"
            "› MTD Overview — Dec 2024"
            "<span style='margin-left:auto;font-size:0.78rem;font-weight:400;"
            "color:#6b7280;background:white;border:1px solid #e5e7eb;"
            "border-radius:6px;padding:4px 10px'>1 Dec – 31 Dec 2024</span>"
        ...
        if "Overview" in data:
            ...
            def kc(m, cls, cur=False, pg=True):
                if m not in ov.index:
                    return (f'<div class="kcell {cls}">'
                            f'<div class="kl">{m}</div><div class="kv">—</div></div>')
                r = ov.loc[m]
        ...
        if "Campaign Performance" in data:
            ...
            with c2:
                ...
                f2.update_layout(height=220, margin=dict(t=0,b=0,l=0,r=45),
            ...
            with c3:
                ...
                f3.update_layout(height=220, margin=dict(t=0,b=0,l=0,r=65),
                                 showlegend=False, paper_bgcolor="white", plot_bgcolor="white",
                                 xaxis=dict(showgrid=False, visible=False),
                                 yaxis=dict(showgrid=False))
    ...
    # ── PAGE 2: TRENDS ───────────────────────────────────────────────────────
        ...
        for i,(lbl,key) in enumerate([("Daily","Daily"),("Weekly","Weekly"),("Monthly","Monthly")]):
        ...
        if st.session_state.gran == "Weekly":
            agg = agg.resample("W",  on="Date").sum().reset_index()
        if st.session_state.gran == "Monthly":
        ...
        # 2x2 chart grid — narrower with gap="large"
        ...
        with r1:
            ...
            st.markdown("</div>", unsafe_allow_html=True)
        with r2:
            st.markdown('<div class="sh">💸 Spend vs Sales</div><div class="sb">',
                        unsafe_allow_html=True)
        ...
        with r3:
            ...
            st.markdown("</div>", unsafe_allow_html=True)
        with r4:
            st.markdown('<div class="sh">🎯 Conversion Funnel</div><div class="sb">',
                        unsafe_allow_html=True)
            fcv = go.Figure()
    ...
    # ── PAGE 3: BY TERRITORY ─────────────────────────────────────────────────
        ...
        with d1:
            ...
            st.markdown("</div>", unsafe_allow_html=True)
        with d2:
            st.markdown('<div class="sh">💰 Sales % of Total</div><div class="sb">',
                        unsafe_allow_html=True)
